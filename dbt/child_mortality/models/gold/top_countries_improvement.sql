...
first_and_last as (
    select
        ...
        round(max(case when year = first_year then mortality_rate end)::numeric, 2) as first_rate,
        round(max(case when year = last_year  then mortality_rate end)::numeric, 2) as last_rate
    from with_bounds
...
select
    ...
    first_rate,
    last_rate,
    round((first_rate - last_rate)::numeric, 2) as improvement
from first_and_last
